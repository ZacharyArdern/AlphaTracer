...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Fetch PDB files from AlphaFold DB and/or ESM Atlas.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument('--afdb_hits', metavar='FILE',
                        help='DIAMOND hits file against AFDB')
    parser.add_argument('--esm_hits', metavar='FILE',
                        help='DIAMOND hits file against ESM Atlas')
    parser.add_argument('--fmt', nargs='+', default=DEFAULT_FMT, metavar='FIELD',
                        help='DIAMOND outfmt 6 column names in order '
                             '(default: standard 12-column fmt 6)')
    parser.add_argument('--pident', type=float, default=0.0,
                        help='Minimum percent identity filter (default: 0, no filter)')
    parser.add_argument('--qcov', type=float, default=0.0,
                        help='Minimum query coverage (qcovhsp) filter (default: 0)')
    parser.add_argument('--targets', type=int, default=1,
                        help='Number of best hits per query, sorted by bitscore '
                             '(or pident if bitscore absent). Default: 1')
    parser.add_argument('--outdir', default='pdb_hits',
                        help='Output directory for PDB files (default: pdb_hits)')
    args = parser.parse_args()

    if not args.afdb_hits and not args.esm_hits:
        parser.error('Provide at least one of --afdb_hits or --esm_hits')
    ...
    if args.afdb_hits:
        hits = parse_hits(args.afdb_hits, fmt, args.pident, args.qcov, 'afdb')
        print(f'AFDB:  {len(hits)} hits after filtering  ({args.afdb_hits})')
        all_hits.extend(hits)

    if args.esm_hits:
        hits = parse_hits(args.esm_hits, fmt, args.pident, args.qcov, 'esm')
        print(f'ESM:   {len(hits)} hits after filtering  ({args.esm_hits})')
        all_hits.extend(hits)
